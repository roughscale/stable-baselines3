...
class SumTree:
    ...
    def add(self, value: float, data: int) -> None:
        ...
        :param data: iDatat for the new leaf node, storing Transition index
            in the case of the prioritized replay buffer
        """
        # Note: transition indices should be constant
    ...
    def get(self, cumulative_sum: float) -> Tuple[int, float, th.Tensor]:
        ...
        :return: Leaf node index, its priority value and transition .
        """
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    :param final_beta: Value of beta at the end of the training
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> PrioritizedReplayBufferSamples:
        ...
        if self.optimize_memory_usage:
            next_obs = self._normalize_obs(
                self.observations[(np.array(sample_indices) + 1) % self.buffer_size, env_indices, :], env
            )
        else:
        ...
        return PrioritizedReplayBufferSamples(*tuple(map(self.to_torch, batch)), leaf_nodes_indices)  # type: ignore
